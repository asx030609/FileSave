...
----针对某列数值，其他列只取一行对应
SELECT * FROM DMT_TAGU TB_A WHERE TB_A.TAG_CODEU = (SELECT MIN(TAG_CODEU) FROM DMT_TAGU  TB_B WHERE TB_A.DATA_TYPEU = TB_B.DATA_TYPEU) ORDER BY TB_A.DATA_TYPEU;
...
SELECT * FROM SYS.ALL_TAB_COLUMNS WHERE TABLE_NAME LIKE '%TM_STORE_OUT_TASKU%' AND "OWNER" = 'C##FUSION';
