...
SELECT * FROM BI_MATCH_PALLETU WHERE BRAND_CODESU='69010281617180001' AND PALLET_TYPE_CODEU='1002';
...
SELECT * FROM WM_PALLETU WHERE LOCATION_CODEU LIKE '%01-12-02%' OR LOCATION_CODEU ='02-08-03' ORDER BY LOCATION_CODEU;
...
SELECT * FROM WM_PALLETU WHERE IN_FROZEN_QUANTITYU > 0 OR OUT_FROZEN_QUANTITYU > 0; --看有哪些盘位正在执行的任务。
----成品库查找某台设备，某个时间点入库物料
SELECT TB_A.EQUIPMENT_USEDU, TB_A.POSITION_USEDU, '  ', TO_CHAR(TB_B.STORAGE_TIMEU, 'yy-mm-dd hh24:mi:ss'),TB_A.LOCATION_NAMEU, TB_A.* 
  FROM WM_LOCATIONU TB_A INNER JOIN WM_STORAGEU TB_B ON TB_B.LOCATION_CODEU = TB_A.LOCATION_CODEU
  WHERE LENGTH(TB_A.LOCATION_CODEU)=8 AND TB_A.POSITION_USEDU !=0 AND TB_A.EQUIPMENT_USEDU LIKE '%01%'
  ORDER BY TB_B.STORAGE_TIMEU DESC, TB_A.EQUIPMENT_USEDU;
...
--SELECT DISTINCT PALLET_IDU FROM WM_STORAGEU WHERE FACTURER_CODEU NOT IN (SELECT BI_FACTURERU.FACTURER_CODEU FROM BI_FACTURERU)
  --SELECT DISTINCT PALLET_IDU FROM WM_STORAGEU WHERE PRODUCT_CODEU='EmptyPallet'
  SELECT DISTINCT PALLET_IDU FROM WM_STORAGEU WHERE STORAGE_QUANTITYU < 30
)--AND MATCH_PALLET_CODEU IS NOT NULL
ORDER BY LOCATION_CODEU;
SELECT STORAGE_QUANTITYU FROM WM_STORAGEU;
...
SELECT * FROM WM_STORAGEU WHERE LOCATION_CODEU='04-32-02';
...
SELECT * FROM TM_TASK_DETAILU WHERE TASK_IDU IN (SELECT IDU FROM TM_TASKU WHERE TASK_NOU=2);
...
--出库任务
SELECT * FROM TM_STORE_OUT_TASKU;
----作业位置
SELECT * FROM TM_WORK_POSITIONU WHERE WORK_POSITION_NOU LIKE '%2101%';
----数据监控数据交互信息交互查询数据标签活动
SELECT * FROM DMT_TAGU;-- WHERE TAG_CODEU LIKE '%Heartbeat_O%';
...
SELECT * FROM DMT_DATA_SERVERU;
SELECT * FROM DMT_GROUPU;
...
----参数设置
SELECT * FROM SYS_PARAMETERU; WHERE NAMEU LIKE '%FailCount%';
SELECT * FROM SYS_PARAMETERU WHERE PARAMETER_NAMEU IN ('FailCount', 'LockTime');
----一列一列理出来获取
SELECT IDU, NAMEU, PARAMETER_NAMEU, PARAMETER_VALUEU, 0 AS PARAMETER_VALUE_TYPEU, GROUP_NAMEU,DISPLAY_ORDERU, CREATE_TIMEU, UPDATE_TIMEU FROM SYS_PARAMETERU;
...
SELECT * FROM WM_STORAGEU WHERE IDENTITY_TAGU IS NOT NULL;
UPDATE WM_STORAGEU SET STORAGE_TIMEU=to_date('2019/11/11', 'yyyy-mm-dd') WHERE IDENTITY_TAGU IS NOT NULL;


----角色权限用户
SELECT * FROM SYS_USERU ;--WHERE USER_NAMEU ='';
SELECT * FROM SYS_PERMISSIONSU;
SELECT * FROM SYS_ROLEU WHERE ROLE_NAMEU LIKE '%mes%' AND ROWNUM=1;
SELECT * FROM SYS_FUNCTIONU ORDER BY MODULE_IDU;
SELECT * FROM SYS_FUNCTIONU WHERE NAMEU LIKE '%刷新%' 
  ORDER BY MODULE_IDU;
----目标是给角色添加所有查询权限，sql语句是：INSERT [dbo].[SYS_Permissions]([Id], [RoleId], [ModuleId], [FunctionId])
SELECT SYS_GUID() IDU,'RoledId',MODULE_IDU,IDU AS FUNCTIONID FROM SYS_FUNCTIONU WHERE NAMEU LIKE '%刷新%';

----查找所有迁移记录映射查找映射迁移查找迁移
SELECT * FROM "MigrationHistory";
SELECT * FROM C##Fusion."MigrationHistory" ORDER By "MigrationId" desc;
