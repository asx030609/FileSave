...
SELECT * FROM WM_MACHINE_PALLETU WHERE  MACHINE_CODEU='B201';--'B205' AND CURRENT_UNIQUE_IDU='03790762' AND ROWNUM=1;
...
SELECT TB_A.TRANSFER_BILL_NOU, TB_A.MACHINE_NAMEU, TB_A.BRAND_NAMEU, TB_A.CURRENT_PLANU, TB_A.NEXT_PLANU, TO_CHAR(TB_A.TRANSFER_TIMEU, 'yy-mm-dd hh24:mi:ss'), TO_CHAR(TB_A.CREATE_TIMEU, 'yy-mm-dd hh24:mi:ss')
  --, TB_A.* 
  FROM WM_MACHINE_HANDOVERU TB_A WHERE CURRENT_PLANU = NEXT_PLANU ORDER BY CREATE_TIMEU DESC, CURRENT_PLANU;
...
SELECT * FROM DMT_INTERACTIVE_DATAU WHERE FIELDU='111' OR FIELDU='112'; --111，112是扫码入库
...
----参数设置
SELECT * FROM SYS_PARAMETERU WHERE NAMEU LIKE '%计划%';
-----update SYS_PARAMETERU set PARAMETER_VALUE_TYPEU='0', DICTIONARY_KEYU='';

---区域和功能设置
SELECT * FROM SYS_AREAU;-- WHERE NAMEU LIKE '%库存%';
SELECT * FROM SYS_MODULEU WHERE NAMEU LIKE '%监测%' OR NAMEU LIKE '%调度%';
SELECT TB_B.NAMEU AS AREA_NAME, TB_A.NAMEU AS MODULE_NAME, TB_A.* 
  FROM SYS_MODULEU TB_A INNER JOIN SYS_AREAU TB_B ON TB_A.AREAU = TB_B.AREA_NAMEU
  WHERE TB_A.NAMEU LIKE '%监测%' OR TB_A.NAMEU LIKE '%预警%';
