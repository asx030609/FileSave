...
SELECT TB_A.*, TB_A.LOCK_TAGU FROM WM_STORAGEU TB_A WHERE LOCATION_CODEU='01-01-02';
SELECT * FROM WM_STORAGEU TB_A WHERE IDU='E6CD5981AAE5433F8656B8F120FEAAA8';
SELECT * FROM WM_STORAGEU WHERE PALLET_IDU IN
  (SELECT IDU FROM WM_PALLETU WHERE PALLET_TYPE_NAMEU LIKE '%%' AND BRAND_NAMESU LIKE '%红旗渠（芒果）%');
SELECT * FROM WM_STORAGEU WHERE FACTURER_NAMEU LIKE '%河南桐裕%'AND LENGTH(LOCATION_CODEU)=8;
SELECT * FROM WM_STORAGEU WHERE IDU='15E5D2658B8F49D9B387BC11A9E5427A';
...
----机组盘位
SELECT * FROM WM_MACHINE_PALLETU WHERE WORK_POSITION_NOU='101001';
...
SELECT * FROM WM_DAILY_BALANCE_BILLU WHERE (ENTRY_QUANTITYU > 0) OR (OUT_QUANTITYU > 0) OR (PROFIT_QUANTITYU > 0) OR (LOSS_QUANTITYU > 0)
