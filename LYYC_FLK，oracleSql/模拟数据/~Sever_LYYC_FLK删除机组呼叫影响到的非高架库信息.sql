...
------删除入库单据DELETE FROM WM_IN_BILL_ALLOT_HISTORYU;
...
----删除盘点单据
DELETE FROM "C##FUSION"."WM_INVENTORY_BILL_DETAILU";
DELETE FROM "C##FUSION"."WM_INVENTORY_BILLU";
----删除日结单据
DELETE FROM WM_DAILY_BALANCE_BILLU;
--*/

----更新前2天之前的所有生产计划状态为已结单
--UPDATE WM_PRODUCTION_PLANU SET PRODUCTION_PLAN_STATUSU=3 WHERE CREATE_TIMEU <= TRUNC( SYSDATE, 'DD' ) -2;
----测试的时候该直接清空信息
...
----更改盘点或其他业务影响到的库存信息
--UPDATE WM_STORAGEU SET LOCK_TAGU='';
UPDATE WM_PALLETU SET PRODUCT_POSITIONU='10102' WHERE LOCATION_CODEU='01-01-02';
UPDATE WM_PALLETU SET STORAGE_SEQUENCEU=abs(STORAGE_SEQUENCEU) WHERE LOCATION_CODEU='01-01-02';
----删除交互信息
